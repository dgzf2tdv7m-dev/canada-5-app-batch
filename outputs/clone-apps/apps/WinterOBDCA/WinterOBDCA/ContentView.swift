...
struct ContentView: View {
    ...
    private let tabs = ["Codes", "Freeze", "Garage"]
    private let tabsFR = ["Codes", "Gel", "Garage"]
    private let features = ["Winter fault library", "Freeze-frame notes", "Battery and heater prompts", "Metric/imperial temperature"]
    private let featuresFR = ["Codes d'hiver", "Notes freeze-frame", "Batterie et chauffage", "Température métrique/impériale"]

    var body: some View {
        ZStack {
            LinearGradient(colors: [Color(hex: "0E1116"), Color(hex: "46A0BE")], startPoint: .topLeading, endPoint: .bottomTrailing)
                .ignoresSafeArea()
            ScrollView {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                Label("WinterOBD CA", systemImage: "car")
            ...
            Text(french ? "Décodez les alertes d'hiver" : "Decode winter warning lights")
                .font(.system(size: 38, weight: .black, design: .rounded))
                .foregroundStyle(foreground)
                .fixedSize(horizontal: false, vertical: true)
            Text(french ? "Référence OBD hivernale pour le Canada : batterie, émissions, ratés, chauffage, ABS et capteurs." : "A cold-weather OBD reference for Canadian drivers: battery, emissions, misfire, heater, ABS, and sensor faults.")
            ...
            HStack {
                ForEach(Array((french ? tabsFR : tabs).enumerated()), id: \.offset) { index, label in
                    Button {
                        ...
                        Label(label, systemImage: ["exclamationmark.triangle", "thermometer.snowflake", "wrench.adjustable"][index])
                            ...
                            .foregroundStyle(selected == index ? Color(hex: "0E1116") : foreground)
                    }
                    .buttonStyle(.plain)
                }
            }
    ...
    private var featureGrid: some View {
        LazyVGrid(columns: [GridItem(.adaptive(minimum: 154), spacing: 14)], spacing: 14) {
            ForEach(Array((french ? featuresFR : features).enumerated()), id: \.offset) { index, text in
                VStack(alignment: .leading, spacing: 14) {
                    Image(systemName: ["car", "exclamationmark.triangle", "thermometer.snowflake", "wrench.adjustable"][index])
    ...
    private var accent: Color { Color(hex: "FF7E2A") }
    private var foreground: Color { Color(hex: "F5F8FA") }
}
